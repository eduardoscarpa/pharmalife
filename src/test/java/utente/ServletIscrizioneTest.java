...
public class ServletIscrizioneTest {
    ...
    @Test
    public void registraUtenteTestAddressIsNotIndex() throws ServletException, SQLException, IOException {
        ...
        assertParametersRequestIsNotIndex(fn,ln,cf,email,psw,psw_rip,via,numeroCivico,cap,telefono);
    }

    @Test
    public void registraUtenteTestAddressIsIndex() throws ServletException, SQLException, IOException {
        when(request.getSession()).thenReturn(s);
        when(request.getParameter("fn")).thenReturn("Matteo");
        when(request.getParameter("ln")).thenReturn("Rispoli");
        when(request.getParameter("cf")).thenReturn("RSPMTT80A01L245X");
        ...
        servletIscrizione.registraUtente(fn,ln,cf,email,psw,psw_rip,via,numeroCivico,cap,telefono,request,response);
        verify(requestDispatcher).forward(request, response);
        assertParametersRequestIsIndex(fn,ln,cf,email,psw,psw_rip,via,numeroCivico,cap,telefono);
    }
    ...
    private void assertParametersRequestIsNotIndex(String fn,String ln,String cf, String email,String psw,String psw_rip,String via, int numeroCivico,String cap,String telefono){
    ...
    private void assertParametersRequestIsIndex(String fn,String ln,String cf, String email,String psw,String psw_rip,String via, int numeroCivico,String cap,String telefono){
        assertEquals("Matteo",fn);
        assertEquals("Rispoli",ln);
        assertEquals("RSPMTT80A01L245X",cf);
        assertEquals("matt@live.it",email);
